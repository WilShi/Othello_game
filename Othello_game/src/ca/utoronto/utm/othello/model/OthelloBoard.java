...
public class OthelloBoard extends BoardVisitable {
	...
	public char hasMove(int row, int col, char player) {
		char retVal = EMPTY;
	 ...
	 * 
	 * @param player P1 or P2
	 * @return the number of tokens in the middle 4 x 4 on the board for player
	 */
	public int getMiddle4By4Count(char player) {
		int[] middle = IntStream.range(2, this.getDimension() - 2).toArray();
		int count = 0;
		for (int row : middle) {
			for (int col : middle) {
				if (board[row][col] == player)
	...
	@Override
	public boolean accept(Visitor visitor, int row, int col) {
